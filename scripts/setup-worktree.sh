...
# 1. Update conda environment name (lines 145, 148, 151) - change "torchcell" to your project name
# 2. Update config file paths (lines 53-55, 66-68) - adjust to your project's config structure
# 3. Update mplstyle path (lines 55, 68) - change "torchcell.mplstyle" to your style file
# 4. Update verification message (line 217) - change "torchcell.__file__" to your package name
...
update_env_paths() {
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        sed -i '' "s|^MPLSTYLE_PATH=.*|MPLSTYLE_PATH=\"$WORKTREE_DIR/Dendron-Template/dendron-template.mplstyle\"|" "$env_file"
...
# Check if conda environment exists - PROJECT-SPECIFIC: change "dendron-template" to your env name
if command -v conda &> /dev/null; then
    if conda env list | grep -q "^dendron-template "; then
        echo "  ✓ dendron-template conda environment exists"
        # Try to get Python path from conda
        CONDA_PYTHON=$(conda run -n dendron-template which python 2>/dev/null || echo "")
        if [ -n "$CONDA_PYTHON" ]; then
            echo "  → Python: $CONDA_PYTHON"
            conda run -n dendron-template python --version
        fi
    else
        echo "  ℹ dendron-template conda environment not found"
        echo "  → Create it with: conda env create -f env/env-dendron-template.yml"
    fi
else
...
echo "  python -c 'import Dendron_Template; print(Dendron_Template.__file__)'"
